...
if(run.feature.train){
tm_xgb.rsh <- system.time(fit_xgb_rsh <- xgb_train(dat_train, label_train))
}
...
if(run.feature.test)
{
  tm_rgb_test <- system.time(rgb_feature_test <- feature_rgb(img_dir_test, export=FALSE))
}
...
# sift
sift_test <- read.csv("../data/test/SIFT_test.csv", header = F) # Load test SIFT features
sift_test <- sift_test[, -1]

# sift(PC)
if(run.feature.test)
{
  tm_pca_sift_test <- system.time(pca_sift_feature_test <- feature_pca(sift_test))
}
...
```{r}
# labels of test data
label_test <- read.csv("../data/test/label_test.csv", header = T)
```
...
xgb.rsh.test <- xgb_test(fit_xgb_rsh, rsh_test) + 1
